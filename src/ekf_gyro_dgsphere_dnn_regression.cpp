...
EKFGyroDgsphereDNN::EKFGyroDgsphereDNN()
	: _nhPrivate("~")
{
	std::cout << "--- ekf_gyro_dgsphere_dnn_regression ---" << std::endl;
	/*parameter*/
	...
	_sub_camera_g = _nh.subscribe("/dnn/g_vector", 1, &EKFGyroDgsphereDNN::callbackCameraG, this);
	/*publisher*/
...
void EKFGyroDgsphereDNN::callbackCameraG(const geometry_msgs::Vector3StampedConstPtr& msg)
{
	/*wait initial orientation*/
	if(!_got_inipose)	return;
	/*observation*/
...
void EKFGyroDgsphereDNN::observationG(geometry_msgs::Vector3Stamped g_msg, double sigma)
{
	/*print*/
	std::cout
...
{
    ros::init(argc, argv, "ekf_gyro_dgsphere_dnn_regression");
	
	EKFGyroDgsphereDNN ekf_gyro_dgsphere_dnn_regression;
