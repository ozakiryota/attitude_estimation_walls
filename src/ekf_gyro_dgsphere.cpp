...
EKFGyroDgsphere::EKFGyroDgsphere()
	: _nhPrivate("~")
{
	std::cout << "--- ekf_gyro_dgsphere ---" << std::endl;
	/*parameter*/
	...
	/*subscriber*/
	_sub_inipose = _nh.subscribe("/initial_orientation", 1, &EKFGyroDgsphere::callbackIniPose, this);
	_sub_imu = _nh.subscribe("/imu/data", 1, &EKFGyroDgsphere::callbackIMU, this);
	_sub_bias = _nh.subscribe("/imu/bias", 1, &EKFGyroDgsphere::callbackBias, this);
	_sub_lidar_g = _nh.subscribe("/lidar/g_vector", 1, &EKFGyroDgsphere::callbackLidarG, this);
	/*publisher*/
...
void EKFGyroDgsphere::callbackIniPose(const geometry_msgs::QuaternionStampedConstPtr& msg)
{
	if(!_got_inipose){
		tf::Quaternion q;
...
void EKFGyroDgsphere::callbackIMU(const sensor_msgs::ImuConstPtr& msg)
{
...
void EKFGyroDgsphere::callbackBias(const sensor_msgs::ImuConstPtr& msg)
{
	if(!_got_bias){
		_bias = *msg;
		_got_bias = true;
...
void EKFGyroDgsphere::callbackLidarG(const geometry_msgs::Vector3StampedConstPtr& msg)
{
	/*wait initial orientation*/
	if(!_got_inipose)	return;
	/*observation*/
...
void EKFGyroDgsphere::observationG(geometry_msgs::Vector3Stamped g_msg, double sigma)
{
	/*print*/
	std::cout
...
void EKFGyroDgsphere::publication(ros::Time stamp)
{
	/*RP*/
...
void EKFGyroDgsphere::estimateYaw(sensor_msgs::Imu imu, double dt)
{
	/*u*/
	Eigen::Vector3d u;
	u <<
...
void EKFGyroDgsphere::getRotMatrixRP(double r, double p, Eigen::MatrixXd& Rot)
{
	Rot <<
...
void EKFGyroDgsphere::getRotMatrixY(double r, double p, Eigen::MatrixXd& Rot)
{
	Rot <<
		0,	sin(r)/cos(p),	cos(r)/cos(p);
}

void EKFGyroDgsphere::anglePiToPi(double& angle)
{
...
{
    ros::init(argc, argv, "ekf_gyro_dgsphere");
	
	EKFGyroDgsphere ekf_gyro_dgsphere;
